...
class CertificatesPage(Gtk.ScrolledWindow):
    ...
    def _setup_ui(self):
        ...
        self.default_info.set_subtitle(_("The Docker container includes pre-configured certificates. You can install Jellyfin immediately without any setup."))
        ...
        self.custom_group.set_description(_("Only needed if you want to use your own certificates"))
        ...
        help_btn.connect("clicked", lambda b: webbrowser.open("https://developer.samsung.com/smarttv/develop/getting-started/setting-up-sdk/creating-certificates.html"))
    ...
    def _update_ui_visibility(self):
        ...
        if use_default:
            ...
            self.continue_row.set_subtitle(_("Ready to install with built-in certificates"))
        else:
    ...
    def _set_certificate_file(self, cert_type, file_path):
        ...
        if cert_type == "author":
            self.author_cert_row.set_subtitle(_("✅ {filename}").format(filename=filename))
            self.author_cert_button.set_label(_("Change"))
        else:
            self.dist_cert_row.set_subtitle(_("✅ {filename}").format(filename=filename))
    ...
    def _check_certificate_completeness(self):
        ...
        if is_complete:
            self.continue_row.set_subtitle(_("Ready to install with custom certificates"))
        else:
            missing = []
            if not author_path:
            ...
            self.continue_row.set_subtitle(_("Missing: {items}").format(items=", ".join(missing)))
